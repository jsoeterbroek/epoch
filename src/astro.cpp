...
std::array<double, 2> ecliptoeq(double jd, double Lambda, double Beta) {
    ...
    double Ra = std::atan2(std::cos(eps) * std::sin(astro::dtr(Lambda)) -
                           std::tan(astro::dtr(Beta)) * std::sin(eps),
                           std::cos(astro::dtr(Lambda)));
    Ra = astro::fixangle(astro::rtd(Ra));
    double Dec = astro::rtd(std::asin(std::sin(eps) * std::sin(astro::dtr(Lambda)) * std::cos(astro::dtr(Beta)) +
                               std::sin(astro::dtr(Beta)) * std::cos(eps)));
    return { Ra, Dec };
}
