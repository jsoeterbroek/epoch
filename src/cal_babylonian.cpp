...
static const char* const BABYLONIAN_MONTHS[14] = {
    "Nisannu", "Ayyaru", "Simanu", "Du'uzu", "Abu", "Ululu",
    "Tashritu", "Aratsamna", "Kislimu", "Ṭebetu", "Shabaatu", "Addaru",
    "Addaru II", "Ululu II"
};

// Weekday names in a standard 7-day cycle
static const char* const BABYLONIAN_WEEKDAYS[7] = {
    "Umu 1", "Umu 2", "Umu 3", "Umu 4", "Umu 5", "Umu 6", "Umu 7"
};
...
std::array<int, 3> jd_to_babylonian(double jd) {
    ...
    while (true) {
        ...
        if (day_of_year < next_month_start || month >= 13) break;
    ...
    if (has_intercalary_month(year)) {
        if (uses_ululu_ii(year) && actual_month == 5) actual_month = 13; // Ululu II
        else if (!uses_ululu_ii(year) && actual_month == 12) actual_month = 12; // Addaru II
        else if (actual_month > 12) actual_month = 13; // shift for Addaru II
